...
class TestRoleManagementFlow:
    ...
    def test_create_user_assign_roles_verify_permissions(self, admin_client):
        ...
        new_user = User.objects.create_user(email="newuser@test.com", password="pass")

        # Step 2: Assign student role
        Role.objects.get_or_create(name="student")
        response = client.post(
            "/api/v1/authorization/assign-role/",
            {"user_id": str(new_user.id), "role_name": "student"},
        )
        ...
        response = client.post(
            "/api/v1/authorization/assign-role/",
            {"user_id": str(new_user.id), "role_name": "content_manager"},
        )
        assert response.status_code == status.HTTP_201_CREATED
        ...
        response = client.post(
            "/api/v1/authorization/remove-role/",
            {"user_id": str(new_user.id), "role_name": "student"},
        )
        assert response.status_code == status.HTTP_200_OK
