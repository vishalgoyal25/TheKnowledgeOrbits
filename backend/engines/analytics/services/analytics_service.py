...
class AnalyticsService:
    ...
    def aggregate_user_day(user: "User", date: date) -> DailyAggregate:
        ...
        # Count articles read
        articles_read = (
            events.filter(event_type="article_read")
        ...
        # Count quizzes taken
        quizzes_taken = events.filter(event_type="quiz_completed").count()

        # Sum quiz scores
        quiz_events = events.filter(event_type="quiz_completed")
        total_score = sum(
            float(event.event_data.get("score", 0)) for event in quiz_events
        )
    ...
    def get_weekly_stats(user: "User") -> Dict[str, Any]:
        ...
        return {
            ...
            "average_score": (
                ...
                if sum(a.quizzes_taken for a in aggregates) > 0
                else 0
            ),
            "daily_data": [
                {
