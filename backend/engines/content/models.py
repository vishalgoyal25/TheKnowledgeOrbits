...
class Document(models.Model):
    ...
    title = models.CharField(
        max_length=500, help_text="Document title (e.g., 'NCERT Class 12 Polity')"
    )

    file_path = models.TextField(help_text="Storage path or URL to the original file")
    ...
    source_version = models.CharField(
        max_length=20, blank=True, help_text="Version number (e.g., '1.0', '2.1')"
    )
...
class Chunk(models.Model):
    ...
    page_number = models.IntegerField(
        null=True, blank=True, help_text="Source page number (for PDFs)"
    )
...
class Embedding(models.Model):
    ...
    vector = VectorField(dimensions=384, help_text="384-dimensional embedding vector")
...
class IngestionJob(models.Model):
    ...
    error_log = models.TextField(blank=True, help_text="Error messages if job failed")
    ...
    processed_pages = models.IntegerField(default=0, help_text="Pages processed so far")
    ...
    started_at = models.DateTimeField(
        null=True, blank=True, help_text="Job start timestamp"
    )

    completed_at = models.DateTimeField(
        null=True, blank=True, help_text="Job completion timestamp"
    )
