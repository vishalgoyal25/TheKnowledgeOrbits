...
class TestDocumentViewSet:
    ...
    def test_list_documents(self, authenticated_admin):
        ...
        Document.objects.create(
            title="Doc 1", file_path="/test1.pdf", source_type="static"
        )
        Document.objects.create(
            title="Doc 2", file_path="/test2.pdf", source_type="static"
        )
    ...
    def test_filter_by_source_type(self, authenticated_admin):
        ...
        Document.objects.create(
            title="Static Doc", file_path="/static.pdf", source_type="static"
        )
        Document.objects.create(
            title="Dynamic Doc", file_path="/dynamic.pdf", source_type="dynamic"
        )
    ...
    @patch(
        "engines.content.services.ingestion_service.IngestionService.ingest_document"
    )
    def test_upload_document(self, mock_ingest, authenticated_admin):
        ...
        data = {"file": pdf_file, "title": "Test Upload", "source_type": "static"}

        response = client.post(
            "/api/v1/content/documents/upload/", data, format="multipart"
        )
...
class TestChunkViewSet:
    ...
    def test_list_chunks(self, authenticated_admin):
        ...
        Chunk.objects.create(
            chunk_text="Chunk 1", chunk_index=0, source_type="static", document=doc
        )
        Chunk.objects.create(
            chunk_text="Chunk 2", chunk_index=1, source_type="static", document=doc
        )
    ...
    def test_filter_chunks_by_document(self, authenticated_admin):
        ...
        doc1 = Document.objects.create(
            title="Doc 1", file_path="/doc1.pdf", source_type="static"
        )
        doc2 = Document.objects.create(
            title="Doc 2", file_path="/doc2.pdf", source_type="static"
        )
