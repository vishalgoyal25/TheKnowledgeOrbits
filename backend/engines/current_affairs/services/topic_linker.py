...
class TopicLinkerService:
    ...
    def link_chunk_to_topics(ca_chunk: CAChunk) -> int:
        ...
        try:
            ...
            if not ca_chunk.embedding_id:
                logger.warning("CA chunk has no embedding", extra={
            ...
            try:
                ...
                logger.error(f"Embedding {ca_chunk.embedding_id} not found for chunk {chunks_id_str}")
                return 0
            ...
            for topic_id, topic_vector in topic_embeddings.items():
                similarity = TopicLinkerService._cosine_similarity(ca_vector, topic_vector)
            ...
            if links_created == 0:
                print(f"DEBUG: Chunk {chunks_id_str} - Max Sim: {max_sim:.4f} with Topic {best_topic_id} (Threshold: {TopicLinkerService.SIMILARITY_THRESHOLD})")
            
            logger.info(f"Linked CA chunk to {links_created} topics", extra={
                'chunk_id': chunks_id_str,
            ...
            logger.error(f"Failed to link CA chunk: {e}", extra={
    ...
    def _get_topic_embeddings() -> Dict[str, np.ndarray]:
        ...
        for topic in topics:
            ...
            chunk_ids = ChunkTopicMap.objects.filter(
                topic=topic
            ).values_list('chunk_id', flat=True)[:10]
            ...
            if chunk_ids:
                embeddings = Embedding.objects.filter(
                    content_type='chunk',
                    content_id__in=[str(cid) for cid in chunk_ids]
                )
    ...
    def link_unlinked_chunks(batch_size: int = 20) -> int:
        ...
        logger.info(f"Processed {processed_chunks_count} chunks, Created {total_links_created} links")
