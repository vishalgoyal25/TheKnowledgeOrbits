...
class RSSScraperService:
    ...
    def scrape_source(source: CASource) -> Dict[str, Any]:
        ...
        try:
            ...
            source.last_error = ''
            source.article_count = CAArticle.objects.filter(source=source).count() # Sync count from DB
            source.save()
            
            logger.info(f"Scraping complete: {articles_new}/{articles_found} new articles")
    ...
    def _process_entry(source: CASource, entry: Dict[str, Any]) -> bool:
        ...
        if not content or len(content.strip()) < 10:
            logger.warning(f"Skipping article '{title}' - No robust content found. (Content len: {len(content) if content else 0})", extra={
                'source': source.name,
                'url': url
            })
            return False
    ...
    def _fetch_full_content(url: str) -> str:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                selectors = ['div.story-details', 'div.full-details', 'article', 'div.article-body', 'div.content', 'div.story_details']
                ...
                if paragraphs:
                     ...
                     valid_ps = [p.get_text().strip() for p in paragraphs if len(p.get_text().strip()) > 50]
                     if valid_ps:
