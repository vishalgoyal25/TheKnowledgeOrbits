...
class CASourceAdmin(admin.ModelAdmin):
    list_display = ['name', 'source_type', 'is_active', 'article_count', 'last_scraped_at', 'status_indicator']
    list_filter = ['is_active', 'source_type', 'scrape_frequency']
    search_fields = ['name', 'url']
    readonly_fields = ['id', 'article_count', 'last_scraped_at', 'last_error', 'created_at', 'updated_at']
    ...
    fieldsets = [
        ...
        ('Statistics', {
            'fields': ['article_count', 'last_scraped_at', 'last_error']
        }),
        ('Timestamps', {
...
class CAArticleAdmin(admin.ModelAdmin):
    list_display = ['title_short', 'source', 'published_at', 'processing_status', 'chunk_count', 'word_count']
    list_filter = ['processing_status', 'source', 'published_at']
    search_fields = ['title', 'content', 'url']
    readonly_fields = ['id', 'word_count', 'chunk_count', 'processed_at', 'created_at', 'updated_at']
    ...
    fieldsets = [
        ...
        ('Content', {
            'fields': ['content', 'summary', 'categories']
        }),
        ('Processing', {
            'fields': ['processing_status', 'processing_error', 'processed_at', 'word_count', 'chunk_count']
        }),
        ('Timestamps', {
            'fields': ['created_at', 'updated_at'],
            'classes': ['collapse']
...
class CAChunkAdmin(admin.ModelAdmin):
    list_display = ['id_short', 'article_title', 'chunk_index', 'published_at', 'is_expired', 'topic_count', 'quality_flag']
    ...
    fieldsets = [
        ...
        ('Quality', {
            'fields': ['quality_flag', 'confidence_score']
        }),
        ('Embedding', {
            'fields': ['embedding_id']
        }),
    ...
    def article_title(self, obj):
        return obj.ca_article.title[:50] + ('...' if len(obj.ca_article.title) > 50 else '')
...
class CATopicLinkAdmin(admin.ModelAdmin):
    list_display = ['id_short', 'ca_article_title', 'topic', 'relevance_score', 'link_method']
    ...
    fieldsets = [
        ('Link Information', {
            'fields': ['id', 'ca_chunk', 'topic']
        }),
        ('Relevance', {
            'fields': ['relevance_score', 'link_method']
        }),
        ('Timestamps', {
            'fields': ['created_at', 'updated_at'],
            'classes': ['collapse']
