        ...
        try:
                ...
                try:
                    ...
                    for emb in embeddings:
                        ...
                        if emb.content_type == 'chunk':
                            try:
                                chunk = Chunk.objects.select_related('document').get(id=emb.content_id)
                                results.append({
                                    'id': str(chunk.id),
                                    'type': 'article',
                        ...
                        # Case B: AI Generated Article
                            ...
                            try:
                                art = GeneratedArticle.objects.select_related('topic').get(id=emb.content_id)
                                results.insert(0, { # Boost to top
                                    'id': str(art.id),
                                    'type': 'article',
                                    'title': art.title,
                                    'snippet': art.summary[:200] + "..." if art.summary else "AI Generated Article",
                        ...
                        # Note: We need to resolve CAChunk -> CAArticle
                             ...
                             try:
                                 ca_chunk = CAChunk.objects.select_related('ca_article', 'ca_article__source').get(id=emb.content_id)
                                 # Deduplicate parent article if already in results ?
                                 if not any(r['id'] == str(ca_chunk.ca_article.id) and r['type'] == 'current_affair' for r in results):
                                     results.append({
                                         'id': str(ca_chunk.ca_article.id), # valid GUID
                                         'type': 'current_affair',
                                         ...
                                         'metadata': {
                                             ...
                                             'date': ca_chunk.published_at.strftime('%Y-%m-%d')
                                         }
                                     })
            ...
            for topic in topics:
                results.append({
                    ...
                    'type': 'topic',
                    'title': topic.name,
                    'snippet': topic.description[:150] + "..." if topic.description else "Explore this topic in depth.",
            ...
            for ca in current_affairs:
                ...
                if not any(r['id'] == str(ca.id) and r['type'] == 'current_affair' for r in results):
                     results.append({
                        'id': str(ca.id),
                        'type': 'current_affair',
                        'title': ca.title,
                        'snippet': ca.summary[:200] + "..." if ca.summary else "Latest update on this topic.",
                        'url': f"/current-affairs/{ca.id}",
                        'metadata': {
                            'source': ca.source.name,
                            'date': ca.published_at.strftime('%Y-%m-%d') if ca.published_at else "Recent"
                        }
                    })

            # --- STRATEGY D: AI Generated Articles Search ---
            # Search actual generated articles first
            generated_articles = GeneratedArticle.objects.filter(
                Q(title__icontains=query) | 
                Q(topic__name__icontains=query) |
                Q(content__icontains=query)  # Search in full article text
            ).filter(is_published=True).select_related('topic')[:5]

            for art in generated_articles:
                # Deduplicate if already found by vector search
                if not any(r['id'] == str(art.id) for r in results):
                    results.insert(0, {  # Insert at top
                        'id': str(art.id),
