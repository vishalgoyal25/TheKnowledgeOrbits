...
class TestCompleteQuizFlow:
    ...
    def test_start_take_submit_flow(self, authenticated_user, topic):
        ...
        quiz = Quiz.objects.create(
            title="Complete Flow Quiz", topic=topic, question_count=3, is_active=True
        )
...
class TestPrivateQuizWorkflow:
    ...
    def test_private_quiz_visibility(
        self, mock_get_generator, authenticated_user, topic
    ):
        ...
        quiz = Quiz.objects.create(
            title="Private Quiz", topic=topic, is_active=True, question_count=10
        )
