...
class TestQuizModel:
    ...
    def test_quiz_ownership_fields(self, topic, user):
        ...
        quiz = Quiz.objects.create(
            title="User Quiz", topic=topic, created_by=user, is_public=False
        )
    ...
    def test_is_user_owned_property(self, topic, user):
        ...
        # User-owned quiz
        quiz1 = Quiz.objects.create(title="Private Quiz", topic=topic, created_by=user)
        assert quiz1.is_user_owned

        # System quiz
        quiz2 = Quiz.objects.create(title="Public Quiz", topic=topic, created_by=None)
        assert not quiz2.is_user_owned

    def test_include_ca_flag(self, topic):
        """Test include_ca flag."""
        quiz = Quiz.objects.create(title="Hybrid Quiz", topic=topic, include_ca=True)
...
class TestQuizAttemptModel:
    ...
    def test_accuracy_property(self, topic, user):
        ...
        attempt = QuizAttempt.objects.create(
            quiz=quiz, user=user, correct_count=8, wrong_count=2, unanswered_count=0
        )
    ...
    def test_accuracy_with_no_answers(self, topic, user):
        ...
        attempt = QuizAttempt.objects.create(
            quiz=quiz, user=user, correct_count=0, wrong_count=0
        )
    ...
    def test_guest_mode_attempt(self, topic):
        ...
        attempt = QuizAttempt.objects.create(quiz=quiz, user=None, status="active")
...
class TestQuestionResponseModel:
    ...
    def test_unique_attempt_question_constraint(self, topic, user):
        ...
        QuestionResponse.objects.create(
            attempt=attempt, question=question, selected_option="A"
        )

        with pytest.raises(Exception):  # IntegrityError
            QuestionResponse.objects.create(
                attempt=attempt, question=question, selected_option="B"
            )
    ...
    def test_unanswered_response(self, topic, user):
        ...
        response = QuestionResponse.objects.create(
            attempt=attempt, question=question, selected_option="", is_correct=False
        )
