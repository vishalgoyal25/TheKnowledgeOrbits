...
from src.cross_subject_map import fuzzy_lookup, SUBJECTS
...
def classify_hierarchy(
    ...
    """
    log_info("  🏛️  Classifier: determining hierarchy...")
    ...
    if topic_name:
        ...
        if map_entry:
            ...
            log_info("     └─ Map hit ✅ (no LLM call needed)")
            log_info(f"     └─ Subject: {result['subject']}")
            log_info(f"     └─ Module:  {result['module']}")
            if result["secondary_subjects"]:
                log_info(
                    f"     └─ Also spans: {', '.join(result['secondary_subjects'])}"
                )
            return result

    # ── Priority 2: LLM Classification (for unknown topics or PDF mode) ───────
    log_info("     └─ Not in map → calling LLM...")
    if ncert_text:
    ...
    if result.get("subject") not in UPSC_SUBJECTS:
        ...
        for known in UPSC_SUBJECTS:
            ...
            log_warning("  ⚠️  Unknown subject returned. Defaulting to Polity.")
    ...
    log_info("     └─ LLM classified ✅")
    log_info(f"     └─ Subject: {result['subject']}")
    log_info(f"     └─ Module:  {result['module']}")
    if result["secondary_subjects"]:
        log_info(f"     └─ Also spans: {', '.join(result['secondary_subjects'])}")
...
def _parse_json(text: str) -> dict:
    ...
    for attempt in [
        ...
        try:
            ...
            pass
    log_warning("  ⚠️  Classifier: JSON parse failed. Using safe defaults.")
    return {
        "subject": "Indian Constitution & Polity",
