...
import time
import wikipedia
from src.llm_client import log_info, log_warning
...
def fetch_full_page(term: str, fallback_suffix: str = "India") -> dict:
    ...
    """
    log_info(f"  🌐 Wikipedia: fetching full page for '{term}'...")

    result = _try_fetch(term)
    if result["found"]:
        log_info(f"     └─ Got {len(result['content'])} chars from '{result['title']}'")
        return result
    ...
    if fallback_suffix and fallback_suffix.lower() not in term.lower():
        ...
        log_warning(f"     └─ Not found. Retrying as '{fallback_term}'...")
        result = _try_fetch(fallback_term)
        if result["found"]:
            log_info(
                f"     └─ Got {len(result['content'])} chars from '{result['title']}'"
            )
            return result

    log_warning(f"     └─ Wikipedia: no page found for '{term}'. Using empty content.")
...
def _try_fetch(term: str) -> dict:
    ...
    try:
        ...
        log_warning(f"     └─ Disambiguation: '{term}' → trying '{best}'")
        try:
        ...
        log_warning(f"     └─ Wikipedia error for '{term}': {str(e)[:60]}")
        time.sleep(3)  # Brief pause on network errors
        return _empty(term)


def extract_relevant_section(
    wiki_content: str, subtopic: str, max_chars: int = 6000
) -> str:
    """
