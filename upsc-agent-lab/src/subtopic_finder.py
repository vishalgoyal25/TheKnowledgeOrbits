...
def find_subtopics(topic_name: str, ncert_text: str = None) -> list:
    ...
    """
    log_info(f"  🔍 Subtopic Finder: discovering all subtopics for '{topic_name}'...")
    ...
    log_info(
        f"     └─ Found {len(subtopics)} subtopics "
        f"({sum(1 for s in subtopics if s['needs_deep'])} marked [DEEP])"
    )
    for s in subtopics:
        deep_flag = " [DEEP]" if s["needs_deep"] else ""
        added_flag = " [added]" if s.get("source") == "added" else ""
        log_info(f"        • {s['name']}{deep_flag}{added_flag}")
...
def find_sub_subtopics(subtopic_name: str, parent_topic: str) -> list:
    ...
    """
    log_info(f"     🔁 Sub-subtopic finder: expanding '{subtopic_name}'...")
...
If "{subtopic_name}" does NOT need further subdivision, return an empty array: []"""
    ...
    log_info(
        f"        └─ {len(sub_subtopics)} sub-subtopics found for '{subtopic_name}'"
    )
...
def _parse_subtopic_list(text: str) -> list:
    ...
    if raw is None:
        log_warning("  ⚠️  Subtopic Finder: could not parse JSON. Returning empty list.")
        return []
